...
int main()
{
    ...
    if(workingMode == "compress")
    {
        ...
        frequencyCounter.readFile("input.txt");                 // changed ../input.txt → input.txt
        huffman.huffer(frequencyCounter.getFrequencyMap());
        huffman.compressTofile("input.txt","output.txt");       // changed ../output.txt → output.txt
    ...
    {
        clock_t tStart = clock();
        huffman.deHuffer("output.txt","output2.txt");           // changed ../output.txt → output.txt
        cout <<"Time taken: "<<(1.0*(clock() - tStart)/CLOCKS_PER_SEC)<<" sec"<<endl;
